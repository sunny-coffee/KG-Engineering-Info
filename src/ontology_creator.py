...
class OntologyCreator:
    ...
    def __init__(self, iri,filename):
        ...
        with onto:
            ...
            class Charateristic(DatatypeProperty):
                domain = [Product]
        onto.save(file=filename)

    def dynamically_add_classes(self, className, input_df: int):
        ...
        with onto:
            ...
            for i in range(int(input_df.shape[0])):
                new_class = types.new_class(input_df.iloc[i,0].replace(" ", "_").replace(",", ""),(getattr(onto,'Charateristic'),))
