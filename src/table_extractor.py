            ...
            if not df_label.empty:
                ...
                for row in  df_label.itertuples():
                    if getattr(row, 'label') != 'Product type' and getattr(row, 'label') != 'Type':
                        ...
                        result = self.extractStream(filename, page_num, getattr(row, 'sub_regions'))
                        if len(result):
                            self.otherList.append(result)
                            # print(result)                  
                    else:
                        resLattice = self.extractLattice(filename, page_num, getattr(row, 'region'))
                        if resLattice != None:
                            if len(resLattice):
                                self.referenceList.append(resLattice)
                                # print(resLattice)
                        else:
                            result = self.extractStream(filename, page_num, getattr(row, 'sub_regions'), 'reference')
                            if len(result):
